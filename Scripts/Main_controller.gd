...
func _process(_delta):
	...
	if step == 1 and has_node("Label") == false:
		...
		dal(format(current_dialogue))
	if step == 2:
		var next_room = str(room_selector())
		#print(next_room)
		load("res://Levels/classroom"+ next_room +".tscn")
		var path = "res://Levels/classroom"+ next_room +".tscn"
		self.queue_free()
...
func format(str1:String, w:int = 100):
	var str_in = str1.split(" ")
	var str_out = ""
	var l_n_pos = 0
	var str_l = 0
	var w_value = 100
